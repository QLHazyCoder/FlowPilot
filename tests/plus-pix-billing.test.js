...
const PIX_STATE = { plusPaymentMethod: 'plus-pix', pixOrderId: '12', pixJobId: 'job-abc', pixTimeoutSeconds: 60 };

test('Pix 轮询 jobs/logs：queued → running → done 判成功并渲染进度日志', async () => {
...
test('Pix 轮询：done_already（账号已是 Plus）也判成功', async () => {
...
test('Pix 轮询：state=failed 时按 error_key 给出友好提示', async () => {
...
test('Pix 轮询：job 404 时回退到 orders 终态查询并成功', async () => {
...
test('Pix 轮询：无 job_id 时直接走 orders 终态轮询', async () => {
  const { executor, completedNodes, fetchedUrls } = createBillingHarness({
    responses: [{ payload: { state: 'done', payment_status: 'paid' } }],
  });
  await executor.executePlusCheckoutBilling({ plusPaymentMethod: 'plus-pix', pixOrderId: '12', pixTimeoutSeconds: 60 });
...
test('Pix 轮询：jobs/logs 非 404 错误时抛错', async () => {
  const { executor } = createBillingHarness({
