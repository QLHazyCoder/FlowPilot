...
test('step 2 phone signup recovers the current tab before reopening the signup entry', async () => {
  const sequence = [];
  const sentPayloads = [];
  const completedPayloads = [];
  ...
  const executor = step2Api.createStep2Executor({
    ...
    ensureContentScriptReadyOnTab: async () => {
      sequence.push('ensureContentScriptReadyOnTab');
    },
    ensureSignupEntryPageReady: async () => {
      sequence.push('ensureSignupEntryPageReady');
      return { tabId: 21 };
    },
    ensureSignupPostIdentityPageReadyInTab: async () => ({
      state: 'phone_verification_page',
    ...
    sendToContentScriptResilient: async (_source, message) => {
      if (message.type === 'ENSURE_SIGNUP_PHONE_ENTRY_READY') {
        sequence.push('ensureSignupPhoneEntryReady');
        if (sequence.filter((entry) => entry === 'ensureSignupPhoneEntryReady').length === 1) {
          return { error: '当前页面没有可用的手机号注册入口，也不在密码页。URL: https://chatgpt.com/' };
        }
    ...
    waitForTabStableComplete: async () => {
      sequence.push('waitForStep2SignupTabToSettle');
    },
  });
  ...
  assert.equal(sequence.includes('ensureSignupEntryPageReady'), false);
  assert.deepStrictEqual(sentPayloads, [
    {
      signupMethod: 'phone',
      phoneNumber: '573504385768',
  ...
  assert.equal(completedPayloads[0]?.payload?.signupPhoneNumber, '573504385768');
});

test('step 2 phone signup reopens signup entry only after current-tab recovery fails', async () => {
  const sequence = [];
  let ensurePhoneEntryCalls = 0;
  ...
  const executor = step2Api.createStep2Executor({
    ...
    sendToContentScriptResilient: async (_source, message) => {
      if (message.type === 'ENSURE_SIGNUP_PHONE_ENTRY_READY') {
        sequence.push('ensureSignupPhoneEntryReady');
        ensurePhoneEntryCalls += 1;
        if (ensurePhoneEntryCalls <= 2) {
          return { error: '当前页面没有可用的手机号注册入口，也不在密码页。URL: https://chatgpt.com/' };
        }
        return { ready: true, state: 'phone_entry', url: 'https://chatgpt.com/' };
      }
