...
test('executeReauthBatch：顺序跑完 3 个账号，全部成功', async () => {
  ...
  const successOnly = JSON.parse(result.successOnlyFileJson);
  assert.equal(successOnly.accounts.length, 3);
  assert.equal(successOnly.accounts[1].credentials.access_token, 'new_access_2');
});
...
test('executeReauthBatch：中间账号失败时 skip 继续，最终统计正确', async () => {
  ...
  const result = await executeReauthBatch({
    accounts,
    mailProvider: '2925',
    originalFileText: JSON.stringify({ source: 'fixture', accounts }),
  });
  ...
  const fullJson = JSON.parse(result.updatedFileJson);
  assert.equal(fullJson.accounts.length, 3);
  assert.equal(fullJson.accounts[1].credentials.email, 'b@2925.com');

  const successOnly = JSON.parse(result.successOnlyFileJson);
  assert.equal(successOnly.source, 'fixture');
  assert.deepEqual(
    successOnly.accounts.map((account) => account.credentials.email),
    ['a@2925.com', 'c@2925.com']
  );
  assert.equal(successOnly.accounts[0].credentials.access_token, 'new_for_a@2925.com');
});
