...
- `phone-sms/providers/registry.js`：接码 provider 注册表，负责统一 `HeroSMS / 5sim / NexSMS / MaDao` 的 ID、显示名、默认顺序、模块装载与实例化入口。
...
- `phone-sms/providers/nexsms.js`：NexSMS 平台适配层，负责国家 ID、服务码、余额、价格查询与 provider registry 工厂契约封装。
- `phone-sms/providers/madao.js`：MaDao 统一接码后端适配层，负责把 FlowPilot 的手机号获取、短信轮询与订单收尾请求映射到 MaDao 的 `POST /api/acquire`、`POST /api/poll` 与 `POST /api/release` 接口，并把 ticket 返回值归一成 FlowPilot 可复用的激活记录结构。
