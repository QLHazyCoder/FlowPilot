...
- `core/flow-kernel/settings-schema.js`：统一设置 schema 与归一化层，负责把持久配置收敛到 `settingsState.services.*` 与 `settingsState.flows.*` 结构，并维护 `activeFlowId`、OpenAI 的 integration target、Kiro/Grok 的 `flows.<flowId>.targetId / targets` 与 `stepExecutionRangeByFlow`；OpenAI 的远程注入开关、URL/Admin Key 与 Grok 远程注入 URL/Admin Key 均归一在 flow 级，不写入 target 级副本。
